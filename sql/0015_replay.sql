-- ============================================================================
-- 0015_replay.sql — gtarp_replay scene storage (the city black-box).
--
-- Two tables, both `gtarp_`-prefixed per the defensive convention adopted
-- after the 0010_properties.sql collision:
--
--   gtarp_replay_scenes       one row per flagged incident. Numeric coord
--                             columns (not JSON) so the nearby-scene query
--                             can bounding-box in SQL instead of parsing.
--   gtarp_replay_participants one row per player captured in a scene; the
...
CREATE TABLE IF NOT EXISTS `gtarp_replay_scenes` (
    ...
    INDEX idx_gtarp_replay_scenes_created (created_at),
    INDEX idx_gtarp_replay_scenes_expires (expires_at),
    INDEX idx_gtarp_replay_scenes_xy (x, y)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS `gtarp_replay_participants` (
    id INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
    ...
    INDEX idx_gtarp_replay_participants_scene (scene_id),
    INDEX idx_gtarp_replay_participants_citizenid (citizenid)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
