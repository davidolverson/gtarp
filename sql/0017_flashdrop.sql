-- ============================================================================
-- 0017_flashdrop.sql — palm6_flashdrop hype-drop economy tables.
--
-- Four tables, all `palm6_`-prefixed per the defensive convention adopted
-- after the 0010_properties.sql collision (see 0012_evidence.sql notes):
--
--   palm6_flashdrop_drops       one row per drop EVENT (a catalog item armed
--                               at a location with a hard supply cap).
--   palm6_flashdrop_serials     the serial REGISTRY — the server-side source
...
--   palm6_flashdrop_provenance  append-only transfer/audit tape per uid:
--                               claim, listing, sale, fence, stolen report,
--                               legit check, counterfeit mint.
--   palm6_flashdrop_listings    the consignment shelf.
--
