-- 0047_yard.sql — palm6_yard (Bolingbroke prison economy)
-- Sentence-shaving labor, buy-only commissary, superlinear bail bonds.
-- Every table is `palm6_`-prefixed per the table-naming convention.
-- Idempotent (CREATE TABLE IF NOT EXISTS) — safe to re-run.
--
-- NOTE: the jail clock itself lives in xt-prison's `xt_prison(identifier,
-- jailtime)` table (created by xt-prison). palm6_yard only READS/writes that row
