-- ============================================================================
-- 0016_clout.sql — tables for gtarp_clout (the IRL-streamer mechanic).
--
-- All tables carry the `gtarp_` prefix per the defensive convention adopted
-- after the 0010_properties.sql collision — the prefix costs nothing and
-- rules out recipe-table collisions permanently.
--
--  * gtarp_clout_streamers — per-character career stats (the leaderboard).
--  * gtarp_clout_deals     — one-time brand-deal unlocks. Payout is
...
--  * gtarp_clout_vod       — the evidence liability: crime events witnessed
--                            while live (who/what/where/when). Police
--                            subpoenas read this; housekeeping prunes it.
-- ============================================================================

CREATE TABLE IF NOT EXISTS `gtarp_clout_streamers` (
...
CREATE TABLE IF NOT EXISTS `gtarp_clout_deals` (
...
CREATE TABLE IF NOT EXISTS `gtarp_clout_vod` (
    id INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
