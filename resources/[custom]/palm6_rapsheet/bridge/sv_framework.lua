-- ============================================================================
-- palm6_rapsheet/bridge/sv_framework.lua
--
...
--
-- Copied from palm6_blotter / palm6_citations bridges and trimmed to what a
...
-- palm6_citations / palm6_blotter Bridge.IsOnDutyPolice exactly, reading the
...
-- Mirrors palm6_blotter Bridge.IsAdmin.
...
-- palm6_citations Bridge.GetCitizenName.
...
function Bridge.Reply(src, lines)
    if src == 0 then
        for _, line in ipairs(lines) do print('[palm6_rapsheet] ' .. line) end
        return
    end
    -- One palm6_ui panel instead of dumping lines into the chat feed.
    TriggerClientEvent('palm6_ui:show', src, { tag = 'Record', color = { 200, 160, 255 }, lines = lines })
end
