-- ============================================================================
-- palm6_ems/shared/config.lua - engine-agnostic tunables (Tier 1, carries to
-- VI). Mirrors the Config.Citation / Config.RateLimits shape from
-- palm6_citations. EMS bills are DEBT WITH MEMORY, same as citations: a bill
...
-- OPTIONAL item gate. palm6_citations requires the 'mdt_tablet' item, but
...
-- path palm6_citations uses for the police account). Absence never blocks
-- settlement.
Config.EmsAccount = 'ambulance'

-- /emscalls read-only dispatch reader over palm6_mdt_calls.
Config.Calls = {
    ListDefault = 6,
    ListMax     = 15,
    -- palm6_mdt_calls has NO status column. "recent" = last N rows within
...
-- Per-source command cooldowns (seconds), mirroring palm6_citations.RateLimits.
Config.RateLimits = {
    emsbill    = 10,
    medbills   = 2,
    paymedbill = 5,
...
-- key because a dedicated feed would require editing palm6_discord's config
