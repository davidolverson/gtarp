# palm6_market — Palm6 Commodity Exchange

A server-authoritative **supply/demand market** for the legal economy's raw
goods. It gives `palm6_grind`'s outputs a living price instead of a flat vendor
rate, and it is the **only buyer** for `animal_pelt` (which `palm6_grind` mints
as a hunting drop but ships no buyer for).

## The loop

Gather raw goods with `palm6_grind` (fish / ore / meat / pelts), then bring them
...
- **`raw_fish`, `raw_ore`, `raw_meat`** can be sold at *either* their fixed
  `palm6_grind` buyer (the safe floor, which also has the grind XP bonus) *or*
...
grow / dry / cook timers in `palm6_drugs`.
...
`palm6_grind`, refine here, sell refined at the exchange — a reason to hold and
...
(`palm6_grind`, gather cooldown) → refine (instant) → sell refined (crashing
curve)*. `palm6_market` is **sell-only** (no buy-back) so there is no
...
running — the same soft-gate discipline as `palm6_drugs`' cook chain.
...
- `sql/0046_market.sql` — `palm6_market_state` (price state) + `palm6_market_trades` (ledger). The refining tier adds **no table** — refined commodities reuse `palm6_market_state`, defaulted to `base` by `seedState()`.
- `palm6_eventguard` budgets `palm6_market:sell` **and `palm6_market:refine`**.
- `palm6_economy` shows an informational **clean-cash** line via the
