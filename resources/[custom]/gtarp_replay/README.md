# gtarp_replay — the city black-box
...
5. **File it.** `/replayattach <id> [note]` files the scene as a
   **REPLAY EXHIBIT** in the `gtarp_evidence` case log, so `/evidence` shows
...
| Robbery | `gtarp_robbery:start` (read-only subscription) | add more hooks in config |
...
- `Config.Triggers` — toggle each trigger; `AutoFlagEvents` subscribes to
  other resources' incident events (ships with `gtarp_robbery:start`).
...
- `Config.Retention` — `Days` (7) and `MaxStoredScenes` (300); pruned on
  start and hourly.
- `Config.EvidenceIntegration` — write exhibits into the `gtarp_evidence`
...
1. Drop `gtarp_replay` into `resources/[custom]/`.
2. Apply `sql/0015_replay.sql` (creates `gtarp_replay_scenes` +
   `gtarp_replay_participants`, both `gtarp_`-prefixed, idempotent
   `CREATE TABLE IF NOT EXISTS`).
3. Add to your server cfg: `ensure gtarp_replay` (after `qbx_core`,
   `ox_lib`, `oxmysql`).
4. Optional: `add_ace group.admin command.replayflag allow` for the staff
   flag command.
5. Restart. Console shows
   `[gtarp_replay] black-box online — 4 Hz ring, 90s window, 7d retention`.
...
- **gtarp_evidence** — `/replayattach` inserts a REPLAY EXHIBIT row into its
  `gtarp_evidence` table (schema from `sql/0012_evidence.sql`); guarded so
  absence degrades gracefully. The resource itself is never modified.
- **gtarp_robbery** — subscribes to its existing `gtarp_robbery:start` net
...
- **gtarp_turf** — no clean incident event exists today (turf tagging is
  non-violent); intentionally not hooked. Any future `gtarp_*` conflict
