...
--      (gtarp_replay:requestBuffer). The client never volunteers a buffer.
--
    ...
    while true do
        ...
        if Game.IsLocalPedShooting() then
            ...
            if Config.Triggers.ShotsFired
                ...
                lastShotReport = now
                TriggerServerEvent('gtarp_replay:shotsFired')
            end
        end
...
RegisterNetEvent('gtarp_replay:requestBuffer', function(captureId)
    if type(captureId) ~= 'number' then return end
    local frames = snapshotRing()
    if #frames < 2 then return end
    TriggerServerEvent('gtarp_replay:uploadBuffer', captureId, frames, Game.GetPedModelHash())
end)
...
RegisterNetEvent('gtarp_replay:playbackMeta', function(meta)
    stopPlayback(true)  -- one reconstruction at a time
    playback.incoming = { meta = meta, parts = {} }
end)

RegisterNetEvent('gtarp_replay:playbackParticipant', function(p)
    local scene = playback.incoming
...
RegisterNetEvent('gtarp_replay:playbackReady', function()
...
RegisterNetEvent('gtarp_replay:showSceneList', function(content)
