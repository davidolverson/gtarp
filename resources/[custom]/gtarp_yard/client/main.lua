    ...
    for _, c in ipairs(Config.Commissary.Items) do
        options[#options + 1] = {
            ...
            onSelect = function()
                ...
                TriggerServerEvent('gtarp_yard:server:buyCommissary', c.item, qty)
            end,
        }
    end
    Game.OpenMenu('gtarp_yard_commissary', Config.Commissary.Label, options)
end
...
local function openBail()
    if Game.Confirm(Config.Bail.Label,
        ...
        TriggerServerEvent('gtarp_yard:server:postBail')
    end
end
    ...
    handles[#handles + 1] = Game.CreateInteraction(
        ...
        function()
            if Game.ProgressBar('Working the yard…', (Config.Labor.TaskSeconds or 6) * 1000) then
                TriggerServerEvent('gtarp_yard:server:doLabor')
            end
        end)
