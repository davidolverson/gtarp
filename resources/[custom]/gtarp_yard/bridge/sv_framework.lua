-- ============================================================================
-- gtarp_yard/bridge/sv_framework.lua
--
-- Framework adapter (server). The ONLY file in this resource that calls
-- qbx_core / ox_inventory / xt-prison / gtarp_mdt exports, the jailTime
-- statebag, or server-side game natives. server/main.lua holds the labor /
-- commissary / bail LOGIC (and its own portable gtarp_yard_* SQL) and calls
...
-- gtarp_market / gtarp_loanshark / gtarp_drugs.
-- ============================================================================
...
-- ---------------------------------------------------------------------------
-- Warrants (gtarp_mdt). Soft cross-calls — nil / false if mdt is absent, never
...
function Bridge.IssueWarrant(citizenid, reason, officerLabel)
    if GetResourceState('gtarp_mdt') ~= 'started' then return nil end
    local ok, id = pcall(function()
        return exports.gtarp_mdt:IssueWarrant(citizenid, reason, officerLabel)
    end)
...
function Bridge.Notify(src, title, msg, t)
    if src == 0 then
        print(('[gtarp_yard] %s: %s'):format(title, msg))
        return
    end
