-- ============================================================================
-- gtarp_yard/server/main.lua — the Bolingbroke prison economy (server-auth).
--
...
-- gtarp_yard_* tables only. No direct framework / native calls here (§6 gate).
--
-- Three server-authoritative net handlers (rate-limited by gtarp_eventguard):
...
--                   flag → gtarp_bounty auto-posts a state contract) and stamp a
...
local function dbg(msg)
    if Config.Debug then print('[gtarp_yard] ' .. msg) end
end
...
-- ===========================================================================
RegisterNetEvent('gtarp_yard:server:doLabor', function()
    local src = source
    ...
    pcall(function()
        MySQL.query.await(
            'INSERT IGNORE INTO gtarp_yard_labor (citizenid, last_task_at, tasks_completed) VALUES (?, 0, 0)',
            { cid })
    end)
    local claimed = 0
    pcall(function()
        claimed = MySQL.update.await(
            'UPDATE gtarp_yard_labor SET last_task_at = ?, tasks_completed = tasks_completed + 1 '
    ...
    -- Sentence baseline (gtarp_yard_sentence). A NEW/longer sentence (current
    ...
    pcall(function()
        row = MySQL.single.await(
            'SELECT baseline_minutes, shaved_minutes FROM gtarp_yard_sentence WHERE citizenid = ?', { cid })
    end)
    if not row then
        pcall(function()
            MySQL.insert.await(
                'INSERT INTO gtarp_yard_sentence (citizenid, baseline_minutes, shaved_minutes, updated_at) '
        ...
        if jailMin > baseline then
            ...
            pcall(function()
                MySQL.update.await(
                    'UPDATE gtarp_yard_sentence SET baseline_minutes = ?, shaved_minutes = 0, updated_at = ? WHERE citizenid = ?',
                    { baseline, t, cid })
            end)
        end
    end
    ...
    -- gtarp_yard_sentence row in place — jailMin never exceeds the stale baseline,
    ...
    if shave > 0 then
        ...
        if Bridge.SetJailMinutes(src, newTime) then
            ...
            pcall(function()
                MySQL.update.await(
                    'UPDATE gtarp_yard_sentence SET shaved_minutes = shaved_minutes + ?, updated_at = ? WHERE citizenid = ?',
                    { applied, t, cid })
            end)
        end
    end
...
-- ===========================================================================
RegisterNetEvent('gtarp_yard:server:buyCommissary', function(item, qty)
    local src = source
    ...
    -- Daily per-item cap (gtarp_yard_commissary_log). Clamp the buy to what is
    -- left today so a big order still partially fills rather than bouncing.
    local ymd = today()
    local sold = 0
    pcall(function()
        local r = MySQL.single.await(
            'SELECT qty FROM gtarp_yard_commissary_log WHERE citizenid = ? AND item = ? AND ymd = ?',
            { cid, item, ymd })
    ...
    pcall(function()
        MySQL.query.await(
            'INSERT INTO gtarp_yard_commissary_log (citizenid, item, ymd, qty) VALUES (?, ?, ?, ?) '
...
-- ===========================================================================
RegisterNetEvent('gtarp_yard:server:postBail', function()
    local src = source
    ...
    -- is re-wanted. gtarp_bounty auto-posts a state contract on its own 180s
    ...
    pcall(function()
        MySQL.insert.await(
            'INSERT INTO gtarp_yard_bail (citizenid, amount, released_minutes, rearrest_until, created_at) '
            .. 'VALUES (?, ?, ?, ?, ?)',
...
exports('RearrestGraceLeft', function(citizenid)
    ...
    pcall(function()
        local r = MySQL.single.await(
            'SELECT MAX(rearrest_until) AS u FROM gtarp_yard_bail WHERE citizenid = ?', { citizenid })
...
-- ===========================================================================
-- boot: self-disable loudly if a required item is missing (mirror gtarp_drugs)
-- ===========================================================================
AddEventHandler('onResourceStart', function(resource)
    ...
    if not Bridge.ResourceStarted('xt-prison') then
        print('^1[gtarp_yard] FATAL: the jail system is not started — the yard has no sentence to work on. Disabled.^0')
        return
    end
    ...
    if #missing > 0 then
        table.sort(missing)
        print(('^1[gtarp_yard] FATAL: %d required item(s) not registered in the item registry — yard disabled. '
            .. 'Add to the shared items file: %s^0'):format(#missing, table.concat(missing, ', ')))
        return
    end

    enabled = true
    print(('[gtarp_yard] prison economy online — labor pay $%d/%ds (shave %d min, cap %d%%), '
...
-- ---------------------------------------------------------------------------
-- Summary export (gtarp_economy / gtarp_devtest — informational, CLEAN cash).
-- ---------------------------------------------------------------------------
exports('GetSummary', function()
    local out = { enabled = enabled, laborTasks = 0, bailsPosted = 0, bailTotal = 0 }
    pcall(function()
        local l = MySQL.single.await('SELECT COALESCE(SUM(tasks_completed),0) AS n FROM gtarp_yard_labor')
        out.laborTasks = l and tonumber(l.n) or 0
        local b = MySQL.single.await('SELECT COUNT(*) AS c, COALESCE(SUM(amount),0) AS s FROM gtarp_yard_bail')
