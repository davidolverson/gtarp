# palm6_insurance — Mors Mutual vehicle policies with forensic fraud detection
...
state-plus-absence, and the fraud score from `palm6_replay`'s black-box
...
`palm6_evidence` case for police to work in RP. Insurance fraud is a story
hook, not a mechanical denial. Case creation lands on the police Discord
feed automatically when `palm6_discord` is configured.
...
| No replay scene near vehicle in last 45 min (damage only, and only when palm6_replay is running) | 35 |
...
- Soft dependencies: palm6_replay missing → no-scene signal disabled
  (absence of the black box is not evidence of fraud); palm6_evidence
...
loop exists nowhere on Tebex — it can't, it needs palm6_replay.
