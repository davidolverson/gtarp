...
local function dbg(msg)
    if Config.Debug then print('[palm6_numbers] ' .. msg) end
end
...
local function runDraw()
    ...
    pcall(function()
        bets = MySQL.query.await(
            "SELECT id, citizenid, number, stake FROM palm6_numbers_bets WHERE draw_seq <= ? AND status = 'open'",
            { seq }) or {}
    end)
    ...
    for _, b in ipairs(bets) do
        ...
        pcall(function()
            local aff = MySQL.update.await(
                "UPDATE palm6_numbers_bets SET status = ?, payout = ? WHERE id = ? AND status = 'open'",
    ...
    pcall(function()
        MySQL.insert.await(
            "INSERT INTO palm6_numbers_draws (draw_seq, winning_number, bets, staked, payout_total) VALUES (?, ?, ?, ?, ?)",
    ...
    print(('[palm6_numbers] draw #%d — winning number %02d (%d bet(s), $%d staked, $%d owed)'):format(
        seq, win, #bets, staked, payoutTotal))
...
local function cmdNumbers(src, args)
    ...
    pcall(function()
        local r = MySQL.single.await(
            "SELECT COUNT(*) AS n FROM palm6_numbers_bets WHERE citizenid = ? AND draw_seq = ?", { cid, seq })
    ...
    local ok = pcall(function()
        MySQL.insert.await(
            "INSERT INTO palm6_numbers_bets (citizenid, number, stake, draw_seq) VALUES (?, ?, ?, ?)",
            { cid, num, stake, seq })
    end)
    if not ok then
...
local function cmdCollect(src)
    ...
    local ok = pcall(function()
        local rows = MySQL.query.await(
            "SELECT id, payout FROM palm6_numbers_bets WHERE citizenid = ? AND status = 'won' AND paid = 0", { cid }) or {}
    ...
    pcall(function()
        local aff = MySQL.update.await(
            "UPDATE palm6_numbers_bets SET paid = 1 WHERE paid = 0 AND id IN (" .. table.concat(placeholders, ',') .. ")",
            ids)
    ...
    if marked ~= #ids then
        ...
        pcall(function()
            MySQL.update.await(
                "UPDATE palm6_numbers_bets SET paid = 0 WHERE id IN (" .. table.concat(placeholders, ',') .. ")", ids)
        end)
        claimLock[cid] = nil
...
local function cmdInfo(src)
    ...
    pcall(function()
        last = MySQL.single.await(
            "SELECT draw_seq, winning_number FROM palm6_numbers_draws ORDER BY draw_seq DESC LIMIT 1")
    end)
    local pending, open = 0, 0
    pcall(function()
        local r = MySQL.single.await(
            "SELECT COALESCE(SUM(payout),0) AS p FROM palm6_numbers_bets WHERE citizenid = ? AND status = 'won' AND paid = 0",
            { cid })
        pending = r and tonumber(r.p) or 0
    end)
    pcall(function()
        local r = MySQL.single.await(
            "SELECT COUNT(*) AS n FROM palm6_numbers_bets WHERE citizenid = ? AND draw_seq = ? AND status = 'open'",
            { cid, openDrawSeq })
...
AddEventHandler('onResourceStart', function(resource)
    ...
    if not Bridge.ItemExists(Config.WinItem) then
        print(('^1[palm6_numbers] FATAL: win item "%s" is not registered in ox_inventory — '
...
exports('GetSummary', function()
    ...
    pcall(function()
        local r = MySQL.single.await(
            "SELECT COUNT(*) AS c, COALESCE(SUM(staked),0) AS s, COALESCE(SUM(payout_total),0) AS p FROM palm6_numbers_draws")
        if r then
