# palm6_courier — player-run delivery board

This is the signature feature for palm6 at Phase 6 of the build roadmap. It
...
- On arrival within `Config.DeliveryRadiusMeters` of the dropoff, the
  client fires `palm6_courier:complete`, the server marks the posting
...
1. Drop a new resource into `resources/[custom]/palm6_<feature>/`.
2. Add an `ensure` line to `custom.cfg` and remove `ensure palm6_courier`.
