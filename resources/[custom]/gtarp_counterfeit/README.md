...
and a police investigation loop built ON the `gtarp_evidence` v2 case
...
6. **The bust.** An officer takes a wad (recipe search/RP), `/seizefake`
   ...
   an idempotent per-batch case via the `gtarp_evidence` v2 exports and
...
- The pen's skill-check pass/fail is client-reported **by design** (same
  trust boundary as gtarp_flashdrop's legit check): it only reveals
...
Six `gtarp_counterfeit_*` tables (`CREATE TABLE IF NOT EXISTS`, no
...
`gtarp_evidence` — this resource stores only *how deep* each serial has
been revealed, so there is no parallel evidence store.

## Install

1. `ensure gtarp_counterfeit` in `custom.cfg` — **after** `ox_inventory`,
   `ox_inventory_overrides`, and `gtarp_evidence`.
...
provenance), `gtarp_evidence` (v2 exports — the serial terminal goes
...
- **`Config.Police`** — terminal coords (keep in sync with
  gtarp_evidence's `LockerCoords`), leads per run/press, interrogate and
