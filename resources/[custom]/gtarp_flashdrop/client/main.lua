-- ============================================================================
-- gtarp_flashdrop/client/main.lua
--
...
local function buildDropSite(payload)
    ...
    dyn.zone = Game.CreateInteraction('drop_table', payload.coords,
        ...
        'fas fa-shoe-prints',
        function() TriggerServerEvent('gtarp_flashdrop:startCheckout') end)
end

RegisterNetEvent('gtarp_flashdrop:stage', function(payload)
...
RegisterNetEvent('gtarp_flashdrop:beginCheckout', function(seconds)
    if Game.ProgressBar('Checking out — do not get robbed', (seconds or 8) * 1000) then
        TriggerServerEvent('gtarp_flashdrop:finishCheckout')
    else
        TriggerServerEvent('gtarp_flashdrop:cancelCheckout')
...
local function openConsignMain()
    Game.OpenMenu('flashdrop_consign', 'SoleWorth Consignment', {
        { title = 'Browse the shelf', description = 'Serialized pairs, verified authentic', icon = 'fas fa-shoe-prints',
          onSelect = function() TriggerServerEvent('gtarp_flashdrop:consign:browse') end },
        { title = 'Consign a pair', description = ('House keeps %d%% of the sale'):format(math.floor(Config.Consignment.FeePct * 100)), icon = 'fas fa-tags',
          onSelect = function() TriggerServerEvent('gtarp_flashdrop:consign:pairs', 'sell') end },
        { title = 'My listings', description = 'Reprice by cancel + relist', icon = 'fas fa-list',
          onSelect = function() TriggerServerEvent('gtarp_flashdrop:consign:myListings') end },
        { title = ('Legit check ($%d)'):format(Config.LegitCheck.Fee), description = 'Registry verdict + provenance tape', icon = 'fas fa-magnifying-glass',
          onSelect = function() TriggerServerEvent('gtarp_flashdrop:consign:pairs', 'legit') end },
        { title = 'Report a pair stolen', description = 'Flags the serial dirty forever', icon = 'fas fa-triangle-exclamation',
          onSelect = function() TriggerServerEvent('gtarp_flashdrop:reportMenu') end },
    })
end
...
function menuBuilders.browse(data)
    ...
    for _, l in ipairs(data) do
        options[#options + 1] = {
            ...
            onSelect = function() TriggerServerEvent('gtarp_flashdrop:consign:buy', l.id) end,
        }
    end
    ...
    for _, p in ipairs(data) do
        options[#options + 1] = {
            ...
            onSelect = function()
                ...
                if price then TriggerServerEvent('gtarp_flashdrop:consign:list', p.uid, price) end
            end,
        }
    end
...
function menuBuilders.legitPairs(data)
    ...
    for _, p in ipairs(data) do
        options[#options + 1] = {
            ...
            onSelect = function() TriggerServerEvent('gtarp_flashdrop:legit:start', p.uid) end,
        }
    end
...
function menuBuilders.myListings(data)
    ...
    for _, l in ipairs(data) do
        options[#options + 1] = {
            ...
            onSelect = function() TriggerServerEvent('gtarp_flashdrop:consign:cancel', l.id) end,
        }
    end
...
function menuBuilders.report(data)
    ...
    for _, p in ipairs(data) do
        options[#options + 1] = {
            ...
            onSelect = function() TriggerServerEvent('gtarp_flashdrop:reportStolen', p.uid) end,
        }
    end
...
function menuBuilders.fence(data)
    ...
    for _, p in ipairs(data) do
        options[#options + 1] = {
            ...
            onSelect = function() TriggerServerEvent('gtarp_flashdrop:fence:sell', p.uid) end,
        }
    end
...
function menuBuilders.craft(data)
    ...
    for _, d in ipairs(data) do
        options[#options + 1] = {
            ...
            icon = 'fas fa-hammer',
            onSelect = function() TriggerServerEvent('gtarp_flashdrop:craft:start', d.dropId) end,
        }
    end
    Game.OpenMenu('flashdrop_craft', 'Counterfeit Bench', options)
end

RegisterNetEvent('gtarp_flashdrop:menuData', function(kind, data)
    local builder = menuBuilders[kind]
    if builder then builder(data or {}) end
end)

RegisterNetEvent('gtarp_flashdrop:beginCraft', function(seconds, label)
    if Game.ProgressBar(('Faking a pair of %s'):format(label or 'sneakers'), (seconds or 12) * 1000) then
        TriggerServerEvent('gtarp_flashdrop:craft:finish')
    else
        TriggerServerEvent('gtarp_flashdrop:craft:cancel')
    end
end)

RegisterNetEvent('gtarp_flashdrop:beginLegit', function(uid)
    local passed = Game.SkillCheck(Config.LegitCheck.Difficulty)
    TriggerServerEvent('gtarp_flashdrop:legit:finish', uid, passed)
end)

RegisterNetEvent('gtarp_flashdrop:report', function(title, content)
...
CreateThread(function()
    ...
    static.handles[#static.handles + 1] = Game.CreateInteraction(
        ...
        function() TriggerServerEvent('gtarp_flashdrop:fence:menu') end)

    -- Counterfeit bench
    static.handles[#static.handles + 1] = Game.CreateInteraction(
        'bench', Config.Counterfeit.Coords, 'Use the workbench', 'fas fa-hammer',
        function() TriggerServerEvent('gtarp_flashdrop:craft:menu') end)

    -- Catch up on any drop already in flight.
    TriggerServerEvent('gtarp_flashdrop:requestSync')
end)
