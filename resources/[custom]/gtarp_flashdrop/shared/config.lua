-- ============================================================================
-- gtarp_flashdrop/shared/config.lua — engine-agnostic tunables (Tier 1,
...
-- minted server-side and registered in gtarp_flashdrop_serials; the registry
...
-- are disabled with a loud console error. `ensure gtarp_flashdrop` must come
...
-- gtarp_turf zone id — when that turf has an owner, the T-5 reveal calls the
-- gang out by name (soft synergy; skipped if gtarp_turf is absent).
-- ---------------------------------------------------------------------------
Config.Locations = {
    {
        id = 'legion_underpass',
    ...
    },
    {
        ...
        -- No turfZone: gtarp_turf has no Rancho/Ballas zone (its zones are
        -- legion_square, grove_street, mirror_park, vinewood, sandy_shores,
        -- paleto_bay). Add a matching zone to gtarp_turf first, then point
...
-- ---------------------------------------------------------------------------
-- Stolen-pair reports write a theft entry into the gtarp_evidence table for
-- detective RP (same soft pattern as gtarp_pumpcoin's rug reveals).
Config.WriteEvidenceOnStolenReport = true

-- When a drop location names a `turfZone` and gtarp_turf shows an owner,
