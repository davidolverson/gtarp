...
active `gtarp_mdt` warrant — funded from nothing, scaled by warrant count,
...
resource only reads `gtarp_mdt_warrants`; it never calls `SealBooking` or
...
separation `gtarp_citations` documents for fine payment vs. warrant
service).
...
## State contracts — read-only cross-read of `gtarp_mdt`

Every sweep (`Config.State.SweepSec`, default 180s) runs `SELECT citizenid,
citizen_name, COUNT(*) FROM gtarp_mdt_warrants WHERE status='active' GROUP
...
capped at `Cap`). This resource **never writes** to `gtarp_mdt`'s tables —
same read-only cross-resource pattern `gtarp_pumpcoin`, `gtarp_clout`, and
`gtarp_flashdrop` already use to read `gtarp_turf`. If `gtarp_mdt` isn't
...
- Bridge-pattern (see `docs/GTA6-READINESS.md`): all logic is in
  ...
  `gtarp_citations`/`gtarp_mdt`.
- Payouts credit by citizenid, online or offline (the `gtarp_insurance`/
  `gtarp_pumpcoin` pattern) so a hunter who logs off between the claim and
...
Same grep across every `resources/[custom]/gtarp_*` README/server file in
this repo also returns nothing — the closest existing systems are
`gtarp_mdt` (warrants are paperwork, no reward), `gtarp_turf` (gang zone
control, not person-targeted), and `gtarp_citations` (debt collection, no
