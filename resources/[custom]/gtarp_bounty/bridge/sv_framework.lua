-- ============================================================================
-- gtarp_bounty/bridge/sv_framework.lua
--
...
-- Display name for board listings.
...
function Bridge.Reply(src, lines)
    for _, line in ipairs(lines) do
        if src == 0 then
            print('[gtarp_bounty] ' .. line)
        else
            TriggerClientEvent('chat:addMessage', src,
                { color = { 200, 40, 40 }, args = { 'Bounty Board', line } })
        end
    end
end
...
-- Credit bank by citizenid, online or offline (pumpcoin/insurance's
...
-- in the logic layer (shared/config.lua owns the number).
