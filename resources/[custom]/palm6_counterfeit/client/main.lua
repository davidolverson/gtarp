-- ============================================================================
-- palm6_counterfeit/client/main.lua
--
...
RegisterNetEvent('palm6_counterfeit:printerSync', function(list)
    clearPrinterZones()
    for _, p in ipairs(list or {}) do
        printerZones[#printerZones + 1] = Game.CreateInteraction(
            ('printer_%d'):format(p.id), p.coords, 'Work the press', 'fas fa-print',
            function() TriggerServerEvent('palm6_counterfeit:printer:menu', p.id) end)
    end
end)

RegisterNetEvent('palm6_counterfeit:printer:menuData', function(d)
    Game.OpenMenu('counterfeit_printer', 'The Press', {
        ...
        },
        {
            ...
            icon = 'fas fa-file',
            onSelect = function() TriggerServerEvent('palm6_counterfeit:printer:feed', d.id, 'paper') end,
        },
        {
            title = ('Feed ink (%d on you)'):format(d.heldInk),
            icon = 'fas fa-fill-drip',
            onSelect = function() TriggerServerEvent('palm6_counterfeit:printer:feed', d.id, 'ink') end,
        },
        {
            title = 'Run a print cycle',
            description = 'Loud on the block — every cycle warms the district',
            icon = 'fas fa-money-bill-wave',
            onSelect = function() TriggerServerEvent('palm6_counterfeit:printer:start', d.id) end,
        },
        {
            title = 'Pack up the press',
            description = 'Hopper contents are LOST',
            icon = 'fas fa-box',
            onSelect = function() TriggerServerEvent('palm6_counterfeit:printer:pickup', d.id) end,
        },
    })
end)

-- Placement: the server asked us to scan for a whitelisted anchor prop.
RegisterNetEvent('palm6_counterfeit:beginPlacement', function()
    ...
    TriggerServerEvent('palm6_counterfeit:place', model)
end)

-- The print cycle: progress bar, then the server verifies the window.
RegisterNetEvent('palm6_counterfeit:beginPrint', function(seconds)
    if Game.ProgressBar('Running plates — stay on the press', (seconds or 20) * 1000) then
        TriggerServerEvent('palm6_counterfeit:printer:finish')
    else
        TriggerServerEvent('palm6_counterfeit:printer:cancel')
...
-- Detector pen
-- ---------------------------------------------------------------------------
RegisterNetEvent('palm6_counterfeit:pen:pick', function(wads)
    local options = {}
    for _, w in ipairs(wads or {}) do
        options[#options + 1] = {
            ...
            onSelect = function() TriggerServerEvent('palm6_counterfeit:pen:start', w.serial) end,
        }
    end
    Game.OpenMenu('counterfeit_pen', 'Detector Pen', options)
end)

RegisterNetEvent('palm6_counterfeit:pen:begin', function(serial)
    local passed = Game.SkillCheck(Config.Pen.Difficulty)
    TriggerServerEvent('palm6_counterfeit:pen:finish', serial, passed)
end)

RegisterNetEvent('palm6_counterfeit:report', function(title, content)
...
-- ---------------------------------------------------------------------------
RegisterNetEvent('palm6_counterfeit:sink:menuData', function(sinkId, wads)
    if #(wads or {}) == 0 then
    ...
    for _, w in ipairs(wads) do
        options[#options + 1] = {
            ...
            onSelect = function() TriggerServerEvent('palm6_counterfeit:sink:spend', sinkId, w.serial) end,
        }
    end
    Game.OpenMenu('counterfeit_sink', 'Pay Cash', options)
end)

RegisterNetEvent('palm6_counterfeit:fence:menuData', function(fenceId, wads, info)
    if #(wads or {}) == 0 then
    ...
    for _, w in ipairs(wads) do
        options[#options + 1] = {
            ...
            onSelect = function() TriggerServerEvent('palm6_counterfeit:fence:pass', fenceId, w.serial) end,
        }
    end
...
-- Police surfaces
-- ---------------------------------------------------------------------------
RegisterNetEvent('palm6_counterfeit:police:pickSeize', function(wads)
    local options = {}
    for _, w in ipairs(wads or {}) do
        options[#options + 1] = {
            ...
            onSelect = function() TriggerServerEvent('palm6_counterfeit:police:bag', w.serial) end,
        }
    end
...
RegisterNetEvent('palm6_counterfeit:heatPing', function(d)
...
RegisterNetEvent('palm6_counterfeit:dispatch', function(d)
    if not d or not d.coords then return end
...
CreateThread(function()
    ...
    for _, sink in ipairs(Config.Sinks) do
        ...
        static.handles[#static.handles + 1] = Game.CreateInteraction(
            ...
            'fas fa-money-bill',
            function() TriggerServerEvent('palm6_counterfeit:sink:menu', sink.id) end)
    end
    ...
    for _, fence in ipairs(Config.Fences) do
        ...
        static.handles[#static.handles + 1] = Game.CreateInteraction(
            ...
            function() TriggerServerEvent('palm6_counterfeit:fence:menu', fence.id) end)
    end

    -- Pick up our own printer zones (server answers only with ours).
    TriggerServerEvent('palm6_counterfeit:requestPrinters')
end)
