-- ============================================================================
-- palm6_counterfeit/bridge/sv_framework.lua
--
...
function Bridge.OnItemMoved(itemName, fn)
    ...
    if not ok then
        print('^3[palm6_counterfeit] WARN: ox_inventory hook API unavailable — '
...
-- server-side calls qbx_police (and the palm6_witnesses fan-in hook)
-- derive the alert/blip coords from arg 3, the suspect's source — the
-- same contract palm6_witnesses' Bridge.PoliceAlert uses. Falls back to
...
function Bridge.PoliceAlert(src, text)
    ...
    -- Fallback: point dispatch to every on-duty officer (palm6_robbery style).
    local ped = GetPlayerPed(src)
    local c = ped and ped ~= 0 and GetEntityCoords(ped) or nil
    if not c then return end
    for _, sid in ipairs(onDutyPolice()) do
        TriggerClientEvent('palm6_counterfeit:dispatch', sid, {
...
function Bridge.PingPoliceArea(coords, radius, label, durationSec)
    for _, sid in ipairs(onDutyPolice()) do
        TriggerClientEvent('palm6_counterfeit:heatPing', sid, {
...
function Bridge.Notify(src, title, msg, t)
    if src == 0 then
        print(('[palm6_counterfeit] %s: %s'):format(title, msg))
        return
    end
