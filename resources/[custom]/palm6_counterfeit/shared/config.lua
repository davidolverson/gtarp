-- ============================================================================
-- palm6_counterfeit/shared/config.lua — engine-agnostic tunables (Tier 1,
...
-- locker terminal turns that chain into named case LEADS (palm6_evidence
...
-- error if any REQUIRED item is missing (same boot gate as palm6_flashdrop).
--
...
-- palm6_flashdrop's craft. Materials are deducted at START (finishing
...
-- palm6_flashdrop's legit check: it only reveals registry truth about a wad
...
-- ---------------------------------------------------------------------------
Config.Police = {
    -- The serial terminal lives AT the palm6_evidence locker (same Mission
    -- Row point — keep in sync with palm6_evidence Config.LockerCoords).
