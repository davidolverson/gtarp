-- ============================================================================
-- palm6_ganginfo/server/main.lua
--
...
--   - palm6_gangs        (0041 + 0043): id, name, tag, rep, created_at,
--                        description. Public identity of each player-run gang.
--   - palm6_gang_members (0041): membership rows; member count is COUNT(*)
--                        grouped / filtered by gang_id.
--   - palm6_turf         (0013): zone ownership; turf held is COUNT(*) WHERE
--                        owner_gang = the gang NAME string (owner_gang stores
--                        the gang's name, not its id, per palm6_turf).
--
-- Two public, rate-limited commands, distinct from palm6_gangs (which owns the
...
local function dbg(msg)
    if Config.Debug then print('[palm6_ganginfo] ' .. msg) end
end
...
-- Normalize a caller-supplied tag exactly the way palm6_gangs sanitizeTag
...
local function memberCount(gangId)
    ...
    pcall(function()
        local r = MySQL.single.await(
            'SELECT COUNT(*) AS c FROM palm6_gang_members WHERE gang_id = ?', { gangId })
        n = r and tonumber(r.c) or 0
    end)
    return n
end

-- Turf zones held. palm6_turf.owner_gang stores the gang NAME string (see
-- palm6_turf/server/main.lua: `z.owner_gang = gang.name`), so we bind the
-- gang's name, not its id. Guarded so an absent palm6_turf yields 0.
local function turfHeld(gangName)
    local n = 0
    pcall(function()
        local r = MySQL.single.await(
            'SELECT COUNT(*) AS c FROM palm6_turf WHERE owner_gang = ?', { gangName })
        n = r and tonumber(r.c) or 0
    end)
    return n
end
    ...
    pcall(function()
        rows = MySQL.query.await([[
            SELECT g.id, g.name, g.tag, g.rep,
                   (SELECT COUNT(*) FROM palm6_gang_members m WHERE m.gang_id = g.id) AS member_count
            FROM palm6_gangs g
...
AddEventHandler('onResourceStart', function(resource)
    ...
    print('[palm6_ganginfo] read-only public gang directory online: /ganginfo <tag>, /gangs [n]')
end)
