-- ============================================================================
-- palm6_ganginfo/bridge/sv_framework.lua
--
...
--
-- Copied from palm6_blotter's bridge and trimmed to what a public read-only
...
function Bridge.Reply(src, lines)
    if src == 0 then
        for _, line in ipairs(lines) do print('[palm6_ganginfo] ' .. line) end
        return
    end
    -- One palm6_ui panel instead of dumping lines into the chat feed.
    TriggerClientEvent('palm6_ui:show', src, { tag = 'Gangs', color = { 170, 130, 255 }, lines = lines })
end
