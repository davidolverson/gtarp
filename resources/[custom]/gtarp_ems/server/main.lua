...
--
-- Debt with memory, same model as gtarp_citations: an EMS bill is a ledger
...
local function dbg(msg)
    if Config.Debug then print('[gtarp_ems] ' .. msg) end
end
...
local function unpaidCount()
    ...
    pcall(function()
        local r = MySQL.single.await(
            "SELECT COUNT(*) AS n FROM gtarp_ems_bills WHERE status = 'unpaid'")
...
local function outstandingSum()
    ...
    pcall(function()
        local r = MySQL.single.await(
            "SELECT COALESCE(SUM(amount), 0) AS total FROM gtarp_ems_bills WHERE status = 'unpaid'")
...
-- ---------------------------------------------------------------------------
-- Boot DDL (self-creating tables). Mirrors the gtarp_dbmigrate pattern:
...
local function ensureSchema()
    local stmts = {
        [[
CREATE TABLE IF NOT EXISTS `gtarp_ems_bills` (
    ...
    INDEX idx_gtarp_ems_bills_patient (patient_citizenid, status),
    INDEX idx_gtarp_ems_bills_medic (medic_citizenid)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
        ]],
        [[
CREATE TABLE IF NOT EXISTS `gtarp_ems_treatments` (
    id INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
    patient_citizenid VARCHAR(64) NOT NULL,
    medic_citizenid VARCHAR(64) NOT NULL,
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_gtarp_ems_treatments_patient (patient_citizenid),
    INDEX idx_gtarp_ems_treatments_medic (medic_citizenid, created_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
        ]],
    }
    for _, sql in ipairs(stmts) do
        ...
        if not ok then
            print(('[gtarp_ems] schema init FAILED -> %s'):format(tostring(err)))
        end
    end
end
...
local function announceBill(billId, patientName, medicName, amount, reason)
    if Config.Discord.Enabled and Bridge.ResourceStarted('gtarp_discord') then
        pcall(function()
            exports.gtarp_discord:Announce(Config.Discord.Feed, {
...
local function cmdEmsBill(src, args)
    ...
    if Config.LogTreatments then
        pcall(function()
            MySQL.insert.await([[
                INSERT INTO gtarp_ems_treatments (patient_citizenid, medic_citizenid, kind, note, bill_id)
                VALUES (?, ?, 'bill', ?, ?)
...
local function cmdPayMedBill(src, args)
    ...
    pcall(function()
        row = MySQL.single.await(
            "SELECT id, amount FROM gtarp_ems_bills WHERE id = ? AND patient_citizenid = ? AND status = 'unpaid'",
            { id, cid })
    end)
    if not row then
    ...
    pcall(function()
        settled = MySQL.update.await(
            "UPDATE gtarp_ems_bills SET status = 'paid', resolved_at = NOW() WHERE id = ? AND status = 'unpaid'",
            { id }) == 1
    end)
    if settled then
...
-- gtarp_mdt_calls (READ-ONLY; this resource never writes that table). The
-- table has no status column, so "recent" is a time window, matching how
-- gtarp_mdt's own /calls reads it.
-- ---------------------------------------------------------------------------
...
local function cmdTreat(src, args)
    ...
    local ok = pcall(function()
        MySQL.insert.await([[
            INSERT INTO gtarp_ems_treatments (patient_citizenid, medic_citizenid, kind, note)
            VALUES (?, ?, 'treat', ?)
    ...
    CreateThread(function()
        ...
        print(('[gtarp_ems] billing open - %d unpaid bill(s), $%d outstanding; dispatch reader %s')
            :format(unpaidCount(), outstandingSum(),
                Bridge.ResourceStarted('gtarp_mdt') and 'ONLINE (gtarp_mdt)' or 'offline'))
    end)
end)
...
-- gtarp_legal reads citation debt via gtarp_citations:GetOpenFor.
exports('GetOpenFor', function(citizenid)
    ...
    pcall(function()
        local r = MySQL.single.await(
            "SELECT COUNT(*) AS n, COALESCE(SUM(amount), 0) AS total FROM gtarp_ems_bills WHERE patient_citizenid = ? AND status = 'unpaid'",
            { citizenid })
        if r then
