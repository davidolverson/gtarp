...
local function tally()
    ...
    local la = Bridge.Summary('palm6_laundering')
    if la then
    ...
    local sz = Bridge.Summary('palm6_seizure')
    if sz then
    ...
    local nu = Bridge.Summary('palm6_numbers')
    if nu then
    ...
    local pr = Bridge.Summary('palm6_protection')
    if pr then
    ...
    local lo = Bridge.Summary('palm6_loanshark')
    if lo then
    ...
    local sm = Bridge.Summary('palm6_smuggling')
    if sm then
    ...
    local dr = Bridge.Summary('palm6_drugs')
    if dr then
    ...
    -- palm6_gangs is not a dirty-cash source or sink (its vault holds CLEAN
    ...
    local ga = Bridge.Summary('palm6_gangs')
    if ga then
    ...
    -- palm6_market pays CLEAN cash for raw goods (legal economy), so like gangs
    -- it doesn't feed the dirty minted/removed tally — informational only.
    local mk = Bridge.Summary('palm6_market')
    if mk then
...
AddEventHandler('onResourceStart', function(resource)
    ...
    print(('[palm6_economy] scoreboard online — /%s aggregates the crime economy for staff'):format(Config.Command))
end)
