...
- **`Config.Locations`** — drop spots: coords + the T-30 riddle + optional
  `turfZone` (a `palm6_turf` zone id for the reveal callout).
...
1. Drop the folder in `resources/[custom]/` and add to `custom.cfg`:
   `ensure palm6_flashdrop` — **after** `ox_inventory` and
...
2. Apply `sql/0017_flashdrop.sql` (creates the four `palm6_flashdrop_*`
...
- **palm6_turf**: drops with a `turfZone` read the zone owner and name the
  gang in the reveal broadcast. Free conflict.
- **palm6_evidence**: stolen-serial reports write a theft entry to the
  evidence table for detective RP (same soft pattern as palm6_pumpcoin's
