-- ============================================================================
-- gtarp_loanshark/bridge/sv_framework.lua
--
-- Framework adapter (server). The ONLY file in this resource that calls
-- qbx_core / ox_inventory / the gtarp_mdt cross-export or server-side natives.
...
-- Soft cross-call into gtarp_mdt: issue a system warrant on a defaulter.
...
function Bridge.HasActiveWarrant(citizenid)
    ...
    local ok, has = pcall(function()
        return exports.gtarp_mdt:HasActiveWarrant(citizenid)
    end)
...
function Bridge.Notify(src, title, msg, t)
    if src == 0 then
        print(('[gtarp_loanshark] %s: %s'):format(title, msg))
        return
    end
