...
(`gtarp_mdt`), which `gtarp_bounty` then auto-posts as a hunting contract. The
...
- **No loan/credit system exists** anywhere. Dup-gated against the real deployed
  ...
  every `gtarp_*` resource.
- **Composes through existing exports, no upstream change.** Default routes
  through `gtarp_mdt:IssueWarrant(cid, reason, 'Loan Shark')`; `gtarp_bounty`'s
  state sweep already auto-posts a contract on any active MDT warrant, so a
  defaulter becomes a bounty target with no direct bounty call. `gtarp_mdt` is
...
- **Feeds the dirty-money economy.** Principal is handed over as `black_money`
  (needs `gtarp_laundering` to spend clean, or spends directly at the recipe's
  BlackMarketArms). Never `counterfeit_cash` (gtarp_counterfeit's lane), never
...
- **Respects the city invariant.** A defaulted debt is settled with the *law*
  ...
  "only a booking clears a warrant" rule `gtarp_citations`/`bounty`/`ransom`
...
`gtarp_loanshark_loans` (`sql/0036_loanshark.sql`) — one row per loan:
