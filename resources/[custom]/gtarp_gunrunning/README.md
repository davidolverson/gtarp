# gtarp_gunrunning — black-market weapon dealer + ballistics tracing
...
server-validated chat command, matching the `gtarp_bounty`/`gtarp_mdt`/
`gtarp_ransom` precedent):
...
`gtarp_ransom`'s flat `Config.Ransom`).
...
2. Looks the real serial up against `gtarp_gunrunning_sales`.
3. On a match, opens/appends a `gtarp_evidence` case (frozen v2 API) and
   links the buyer as a suspect via `LinkSuspect` — a ballistics match.

Deliberately **no auto-warrant** here (unlike `gtarp_ransom`, which does
...
- `gtarp_evidence` integration uses only the frozen four exports
  ...
  tables directly.
- `/buyweapon` is a chat command, not a net event — `gtarp_eventguard`'s
...
both `resources/[custom]` (every existing `gtarp_*` resource) and the
...
`gtarp_counterfeit`'s README (a different concept — fake cash, not guns).
No existing `gtarp_*` resource sells weapons or tracks serial provenance.
