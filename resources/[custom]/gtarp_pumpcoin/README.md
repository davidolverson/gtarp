# gtarp_pumpcoin — the back-alley memecoin exchange
...
4. **Rug** — the creator holds a hidden **dev wallet** (250 units premined at
   ...
   **10-minute anonymity window** the creator's identity is revealed
   ...
   (`gtarp_evidence`). Robbable/chargeable under server rules from there.
...
- `VerifiedEnabled`, `VerifiedTurfZones` — gtarp_turf synergy.
- `Name*`/`Ticker*` limits, `Emojis` whitelist, `ChartTradeLimit`.

## Install

1. `ensure gtarp_pumpcoin` in `custom.cfg` (after `qbx_core`, `ox_lib`,
   `oxmysql`).
2. Apply `sql/0014_pumpcoin.sql` — creates `gtarp_pumpcoin_coins`,
   `gtarp_pumpcoin_holdings`, `gtarp_pumpcoin_trades`.
...
- **gtarp_turf** — if the creator's gang owns ≥2 turf zones at mint, the coin
  lists with a **VERIFIED** badge. Turf dominance as buyer confidence.
- **gtarp_evidence** — rug reveals write an automated fraud entry for
...
- **Renewed-Banking / qbx money** — all settlement is the framework bank via
  ...
  credit path gtarp_courier uses.
- **gtarp_grind / gtarp_courier** — the 2% fee + mint cost sink the cash
...
gtarp_evidence). Server housekeeping is one 30s sweep. Trades are 3 queries
