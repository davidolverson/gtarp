-- ============================================================================
-- gtarp_market/bridge/sv_framework.lua
--
...
-- gtarp_market_* SQL) ports to GTA VI by rewriting THIS FILE.
-- See docs/GTA6-READINESS.md (Section 3, the bridge pattern).
--
-- Item / cash / coords helpers are the exact shapes used by gtarp_grind; the
-- Reply-to-panel helper is the shape used by gtarp_help / gtarp_economy.
-- ============================================================================
...
-- single branded gtarp_ui panel instead of several lines dumped into chat.
function Bridge.Reply(src, lines)
    if src == 0 then
        for _, l in ipairs(lines) do print('[gtarp_market] ' .. tostring(l)) end
        return
    end
    local c = Config.Panel.color
    TriggerClientEvent('gtarp_ui:show', src, {
