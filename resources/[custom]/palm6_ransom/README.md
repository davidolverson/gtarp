# palm6_ransom — kidnapping ransom ledger
...
into `palm6_evidence` and `palm6_mdt`.
...
**Every case closes into an `palm6_mdt` warrant, paid or not.** Paying the
ransom buys the victim's release, not the kidnapper's freedom — same
"paying doesn't clear the underlying charge" separation `palm6_citations`
(fine payment doesn't lift a warrant) and `palm6_bounty` (capture doesn't
...
Paying is guarded the same way `palm6_bounty`'s capture and
`palm6_pumpcoin`'s mint are: `UPDATE ... SET status='paid' WHERE
...
- Bridge-pattern (see `docs/GTA6-READINESS.md`): all logic is in
  ...
  client script — matches the `palm6_bounty`/`palm6_mdt`/`palm6_citations`
...
- `palm6_evidence` integration uses only the frozen four exports
  ...
  tables directly.
- `palm6_mdt` integration uses only its additive `IssueWarrant` export
  (the same call `palm6_citations`' overdue-fine escalation uses) — never
...
- Payouts credit by citizenid, online or offline (the `palm6_bounty`/
  `palm6_insurance`/`palm6_pumpcoin` pattern).
...
worked around, same style as `palm6_mdt`'s known truckrobbery/cam-command
alert-funnel gap.

## Dup-gate (2026-07-08)

`grep -riE "kidnap|hostage|ransom"` across both `resources/[custom]` (every
existing `palm6_*` resource) and the deployed recipe's full `[qbx]` tree
...
database, or a case file). No existing `palm6_*` resource does ransom or
hostage-economy logic. Same shape as the gap `palm6_mdt` closed for the
