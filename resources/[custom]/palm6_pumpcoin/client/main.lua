-- ============================================================================
-- palm6_pumpcoin/client/main.lua
--
...
-- standing on a terminal (same distance-gated pattern as palm6_evidence —
    ...
    while true do
        ...
        elseif nearDist <= Config.InteractRadius then
            ...
            if Game.InteractPressed() then
                TriggerServerEvent('palm6_pumpcoin:requestOpen')
            end
        end
...
-- Server -> NUI
-- ---------------------------------------------------------------------------
RegisterNetEvent('palm6_pumpcoin:open', function(payload)
    uiOpen = true
...
RegisterNetEvent('palm6_pumpcoin:data', function(payload)
    if not uiOpen then return end
    payload.action = 'data'
    Game.SendUIMessage(payload)
end)

RegisterNetEvent('palm6_pumpcoin:chart', function(payload)
    if not uiOpen then return end
...
RegisterNetEvent('palm6_pumpcoin:rugged', function()
    if uiOpen then TriggerServerEvent('palm6_pumpcoin:requestData') end
end)
...
RegisterNUICallback('refresh', function(_, cb)
    TriggerServerEvent('palm6_pumpcoin:requestData')
    cb({ ok = true })
end)

RegisterNUICallback('chart', function(data, cb)
    TriggerServerEvent('palm6_pumpcoin:requestChart', { coinId = tonumber(data and data.coinId) })
    cb({ ok = true })
end)

RegisterNUICallback('mint', function(data, cb)
    TriggerServerEvent('palm6_pumpcoin:mint', {
...
RegisterNUICallback('buy', function(data, cb)
    TriggerServerEvent('palm6_pumpcoin:buy', {
...
RegisterNUICallback('sell', function(data, cb)
    TriggerServerEvent('palm6_pumpcoin:sell', {
        coinId = tonumber(data and data.coinId),
        units = tonumber(data and data.units),
    })
    cb({ ok = true })
...
-- Billboard blips
-- ---------------------------------------------------------------------------
RegisterNetEvent('palm6_pumpcoin:billboardAdd', function(id, coords, label)
    if BillboardBlips[id] then Game.RemoveBlip(BillboardBlips[id]) end
    BillboardBlips[id] = Game.CreateBillboardBlip(coords, label)
end)

RegisterNetEvent('palm6_pumpcoin:billboardRemove', function(id)
...
RegisterNetEvent('palm6_pumpcoin:billboardSync', function(list)
...
CreateThread(function()
    Wait(3000)
    TriggerServerEvent('palm6_pumpcoin:requestBillboards')
end)
