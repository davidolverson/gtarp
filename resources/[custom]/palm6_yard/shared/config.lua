...
--                (palm6_bounty then auto-posts a state contract on its sweep),
...
--   * CooldownSec is PERSISTED (palm6_yard_labor) so relog-to-reset is blocked.
...
-- item + qty only). Markup + a daily per-item cap (palm6_yard_commissary_log)
...
-- and re-issues an mdt warrant (the skip flag) — palm6_bounty auto-posts the
...
-- palm6_yard self-disables LOUDLY at boot if any are missing (mirrors
-- palm6_drugs). Every commissary item must be in this list.
-- ---------------------------------------------------------------------------
