# palm6_yard — Bolingbroke prison economy
...
- **Bail** = `Base + floor(remainingSeconds^1.15 * K)`, floored at `Floor` (above
  a typical crime payout). Releasing re-issues `palm6_mdt:IssueWarrant`; on the
  next 180s sweep `palm6_bounty` auto-posts a city-funded state contract — we
  wire nothing there.

## Money-safety
- Atomic per-player cooldown set BEFORE any yield (in-memory same-tick guard +
  a **persisted** `palm6_yard_labor.last_task_at` gate that survives relog).
...
- `server/main.lua` / `client/main.lua` = **logic only**, calling `Bridge.*` /
  `Game.*`. `MySQL.*` for the resource's own `palm6_yard_*` tables is allowed in
  server logic.
- `bridge/sv_framework.lua` — the ONLY server file touching qbx_core,
  ox_inventory, the xt-prison export/statebag, palm6_mdt, and natives.
- `bridge/cl_game.lua` — the ONLY client file touching natives / ox_target /
  ox_lib.

## Tables (all `palm6_`-prefixed) — migration `sql/0047_yard.sql`
`palm6_yard_sentence`, `palm6_yard_labor`, `palm6_yard_commissary_log`,
`palm6_yard_bail`.

## New ox_inventory items (owned by this resource)
`yard_pruno`, `yard_commissary_snack`, `yard_soap`. They are NOT edited into
`items.lua` by this resource — the orchestrator adds them. **palm6_yard
self-disables loudly at boot if any is missing** (mirrors palm6_drugs).
