...
-- ---------------------------------------------------------------------------
RegisterNetEvent('gtarp_clout:streamStarted', function(payload)
    Game.SendUIMessage({ action = 'open', viewers = payload.viewers or 0 })
end)

RegisterNetEvent('gtarp_clout:tick', function(payload)
    Game.SendUIMessage({
        action = 'tick',
...
RegisterNetEvent('gtarp_clout:donation', function(payload)
    Game.SendUIMessage({
        action = 'donation',
...
RegisterNetEvent('gtarp_clout:milestone', function(payload)
    Game.SendUIMessage({
        action = 'milestone',
        viewers = payload.viewers or 0,
...
RegisterNetEvent('gtarp_clout:streamEnded', function(payload)
    Game.SendUIMessage({
        action = 'ended',
...
RegisterNetEvent('gtarp_clout:showVodLog', function(title, content)
...
RegisterNetEvent('gtarp_clout:liveAdd', function(serverId, name)
    LiveStreamers[serverId] = name or true
    refreshAnyLive()
end)

RegisterNetEvent('gtarp_clout:liveRemove', function(serverId)
    LiveStreamers[serverId] = nil
    dropTag(serverId)
    refreshAnyLive()
end)

RegisterNetEvent('gtarp_clout:liveSync', function(list)
    LiveStreamers = {}
...
CreateThread(function()
    Wait(3000)
    TriggerServerEvent('gtarp_clout:requestLiveSync')
end)
...
-- gtarp_evidence — no unconditional per-frame work on a 48-slot server).
-- ---------------------------------------------------------------------------
CreateThread(function()
    local brokerPed = nil
    while true do
        ...
        if dist <= Config.InteractRadius then
            ...
            if Game.InteractPressed() then
                TriggerServerEvent('gtarp_clout:requestClaimDeals')
            end
        end
