# gtarp_clout — go live, chase clout, become evidence
...
4. **Become evidence** — everything scored is also written to your VOD:
   ...
   (`gtarp_evidence`). Gangs now have a mechanical reason to chase streamers
...
- `DangerZone*`, `TurfRefreshSec`, `DangerZones` — gtarp_turf synergy (zone
  ids/coords must mirror gtarp_turf's `Config.Zones`).
...
1. `ensure gtarp_clout` in `custom.cfg` (after `qbx_core`, `ox_lib`,
   `oxmysql`).
2. Apply `sql/0016_clout.sql` — creates `gtarp_clout_streamers`,
   `gtarp_clout_deals`, `gtarp_clout_vod`.
...
4. Tune `shared/config.lua` (at minimum check the pawnshop coords fit your
   map edits, and that `DangerZones` mirrors your gtarp_turf zones).

## Synergies (all soft dependencies — degrade silently if absent)

- **gtarp_evidence** — every served subpoena files a case-log entry
  (`/evidence` shows it); the description points detectives at the full tape
  in `gtarp_clout_vod`.
- **gtarp_turf** — streaming on a gang-OWNED zone multiplies viewer gains
  (ownership read live from the `gtarp_turf` table). Streamers hunt owned
...
and only tightens at the counter (gtarp_evidence pattern); the LIVE-tag loop
