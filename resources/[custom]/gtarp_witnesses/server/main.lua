-- ============================================================================
-- gtarp_witnesses/server/main.lua
--
-- Living NPC witnesses for every crime. An event bus (weaponDamageEvent,
-- the recipe's police:server:policeAlert fan-in, gtarp_robbery, plus a
...
-- Police canvass witnesses to feed a gtarp_evidence v2 case file; the
...
-- gtarp_witnesses_* SQL is portable, so it stays here (see
...
-- FROZEN gtarp_evidence v2 exports (EnsureCase / AppendEntry /
...
local function dbg(msg)
    if Config.Debug then print('[gtarp_witnesses] ' .. msg) end
end
...
-- Per-citizen cooldowns, gtarp_pumpcoin pattern: check-and-consume returns
...
local function syncOne(src)
    ...
    TriggerClientEvent('gtarp_witnesses:sync', src,
...
local function persistIncident(inc, witnessDrafts)
    local okInc, incidentId = pcall(function()
        return MySQL.insert.await(
            'INSERT INTO gtarp_witnesses_incidents (uid, crime, label, suspect_citizenid, x, y, z, fact_pool, created_at, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    ...
    for _, d in ipairs(witnessDrafts) do
        local okW, wid = pcall(function()
            return MySQL.insert.await(
                'INSERT INTO gtarp_witnesses (incident_id, x, y, z, facts, status) VALUES (?, ?, ?, ?, ?, ?)',
    ...
    if #ids == 0 then
        pcall(function()
            MySQL.update.await('DELETE FROM gtarp_witnesses_incidents WHERE id = ?', { incidentId })
        end)
        return nil
    end
...
local function reportCrime(src, hook, witnessCoordsOverride)
    ...
    TriggerClientEvent('gtarp_witnesses:captureAppearance', src, nonce)
...
RegisterNetEvent('gtarp_witnesses:appearanceResult', function(nonce, sig)
    local src = source
...
-- gtarp_robbery ATM hold-ups (custom layer). SERVER-ONLY listener
-- (AddEventHandler, never RegisterNetEvent): gtarp_robbery fires this via
...
-- 'gtarp_robbery:start' net event would create witnesses for rejected —
-- or entirely forged — robberies.
if Config.Hooks.gtarpRobbery and Config.Hooks.gtarpRobbery.enabled then
    AddEventHandler(Config.Hooks.gtarpRobbery.event, function(robberSrc)
        reportCrime(robberSrc, Config.Hooks.gtarpRobbery)
    end)
end

-- Sibling resources can feed the bus directly:
--   exports.gtarp_witnesses:ReportCrime(src, 'arson', 'a fire being set', false)
...
-- ---------------------------------------------------------------------------
-- Case-file plumbing (gtarp_evidence v2 FROZEN exports — the only write
-- path; we never touch gtarp_evidence tables and keep no parallel store).
-- ---------------------------------------------------------------------------
local function ensureCaseFor(inc)
    if inc.caseId then return inc.caseId end
    local ok, caseId = pcall(function()
        return exports.gtarp_evidence:EnsureCase(
    ...
    inc.caseId = caseId
    pcall(function()
        MySQL.update.await('UPDATE gtarp_witnesses_incidents SET case_id = ? WHERE id = ?',
            { caseId, inc.id })
    end)
    return caseId
end
...
local function appendTestimony(inc, w, facts, officerName)
    ...
    local ok, entryId = pcall(function()
        return exports.gtarp_evidence:AppendEntry(caseId, 'fact', statement, RESOURCE_TAG)
    end)
    if not ok or not entryId then return false end
    pcall(function()
        exports.gtarp_evidence:LinkSuspect(caseId, nil, table.concat(texts, ', '))
    end)
    return true
end
...
local function setWitnessStatus(w, status, byCid, corrupted)
    ...
    pcall(function()
        MySQL.update.await(
            'UPDATE gtarp_witnesses SET status = ?, status_by = ?, corrupted_facts = ? WHERE id = ?',
            { status, byCid, w.corrupted and json.encode(w.corrupted) or nil, w.id })
    end)
end

RegisterNetEvent('gtarp_witnesses:canvass:start', function(wid)
    local src = source
    ...
    TriggerClientEvent('gtarp_witnesses:beginCanvass', src, Config.Canvass.DurationSec)
end)

RegisterNetEvent('gtarp_witnesses:canvass:finish', function()
    local src = source
    ...
    -- Claim the witness BEFORE the yielding gtarp_evidence export calls in
            ...
            body = ('"Okay... okay. Here\'s what I saw."\n\nThe suspect %s.\n\n%s')
                :format(table.concat(factTexts(w.corrupted), '; '),
                    ok and ('_Statement logged to case #%d._'):format(inc.caseId or 0)
                       or '_Failed to log the statement — check gtarp_evidence._')
        else
            title = 'Witness Canvass'
    ...
    w.busy = nil
    TriggerClientEvent('gtarp_witnesses:showStatement', src, title, body)
    syncAll()
end)

RegisterNetEvent('gtarp_witnesses:canvass:cancel', function()
...
RegisterNetEvent('gtarp_witnesses:press:start', function(wid)
    local src = source
    ...
    TriggerClientEvent('gtarp_witnesses:beginPress', src, Config.Press.AimSec)
end)

RegisterNetEvent('gtarp_witnesses:press:finish', function()
    local src = source
...
RegisterNetEvent('gtarp_witnesses:press:cancel', function()
    pendingPress[source] = nil
end)

RegisterNetEvent('gtarp_witnesses:payoff', function(wid)
    local src = source
...
RegisterNetEvent('gtarp_witnesses:requestSync', function()
    local src = source
...
RegisterCommand('witnesses', function(src, args)
    ...
    if sub == 'sim' then
        if src == 0 then
            print('[gtarp_witnesses] sim needs an in-game admin')
            return
        end
...
-- ---------------------------------------------------------------------------
-- Boot: presence-check the gtarp_evidence v2 export surface (loud, not
...
AddEventHandler('onResourceStart', function(resource)
    ...
    local okApi = pcall(function()
        ...
        exports.gtarp_evidence:EnsureCase(nil, nil, nil)
    end)
    if not okApi then
        print('^1[gtarp_witnesses] WARNING: gtarp_evidence v2 exports are unreachable. '
            .. 'Canvassed statements will NOT reach case files until gtarp_evidence '
    ...
    pcall(function()
        local incRows = MySQL.query.await(
            'SELECT * FROM gtarp_witnesses_incidents WHERE expires_at > ?', { t }) or {}
        ...
        local witRows = MySQL.query.await(
            "SELECT w.* FROM gtarp_witnesses w JOIN gtarp_witnesses_incidents i ON i.id = w.incident_id WHERE i.expires_at > ? AND w.status <> 'canvassed'",
            { t }) or {}
    ...
    print(('[gtarp_witnesses] ready — %d live incident(s), %d witness(es) reloaded; alerts %s')
