...
layered onto crimes the city already commits, feeding the `gtarp_evidence`
...
1. **The crime.** The event bus hooks the server's `weaponDamageEvent`
   ...
   corner selling, jewelery, house/bank robbery), and `gtarp_robbery` ATM
...
3. **The canvass.** Witness markers persist ~30 minutes (they survive
   ...
   `gtarp_evidence` **case file** (one case per incident, created lazily on
...
- **Case files via the frozen `gtarp_evidence` v2 exports** (`EnsureCase` /
  `AppendEntry` / `LinkSuspect`) — this resource never writes to
  `gtarp_evidence` tables and keeps no parallel evidence store. Witness
  state itself lives in its own `gtarp_witnesses*` tables.
- **Rate limiting:** `gtarp_eventguard` exposes no registration export for
  new events, so every client-triggerable event here carries its own
  per-source rate limit **and** per-citizen cooldown (the `gtarp_pumpcoin`
...
| `/evidence case <id>` | police | read accumulated witness statements (gtarp_evidence) |
...
1. Drop the folder in `resources/[custom]/` and add to `custom.cfg`:
   `ensure gtarp_witnesses` — **after** `gtarp_evidence` (its frozen v2
   ...
   loudly).
2. Apply `sql/0019_witnesses.sql` (creates the two `gtarp_witnesses*`
...
Requires: `qbx_core`, `ox_lib`, `oxmysql`, `gtarp_evidence` (v0.2.0+),
OneSync (the server-side ped snapshot reads the ambient population).

## Synergies

- **gtarp_evidence** (hard, via frozen exports): every canvass appends a
...
- **gtarp_robbery** (soft): ATM hold-ups leave witnesses too.
