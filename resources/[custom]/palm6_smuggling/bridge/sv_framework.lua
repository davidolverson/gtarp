-- ============================================================================
-- palm6_smuggling/bridge/sv_framework.lua
--
-- Framework adapter (server). The ONLY file in this resource that calls
-- qbx_core / ox_inventory / qbx_police / palm6_evidence exports or server-side
...
-- smuggling into the dirty-money economy (launderable via palm6_laundering,
-- forfeitable via palm6_seizure). NOT markedbills (qbx_drugs' item).
...
function Bridge.PoliceAlert(src, text)
    ...
    for _, sid in ipairs(onDutyPolice()) do
        TriggerClientEvent('palm6_smuggling:dispatch', sid, { coords = { x = c.x, y = c.y, z = c.z }, label = text })
    end
end

-- Soft palm6_evidence v2 hooks (frozen exports; never its tables).
function Bridge.EvidenceEnsureCase(incidentKey, title, createdBy)
    if GetResourceState('palm6_evidence') ~= 'started' then return nil end
    local ok, id = pcall(function() return exports.palm6_evidence:EnsureCase(incidentKey, title, createdBy) end)
    return (ok and id) or nil
end

function Bridge.EvidenceAppend(caseId, kind, payload, source)
    pcall(function() exports.palm6_evidence:AppendEntry(caseId, kind, payload, source) end)
end

function Bridge.EvidenceLinkSuspect(caseId, citizenid, descriptor)
    pcall(function() exports.palm6_evidence:LinkSuspect(caseId, citizenid, descriptor) end)
end
...
function Bridge.Notify(src, title, msg, t)
    if src == 0 then print(('[palm6_smuggling] %s: %s'):format(title, msg)); return end
...
-- Soft hype broadcast to the palm6_discord announcer (tolerated absent).
