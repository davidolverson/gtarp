...
then has to be laundered (`gtarp_laundering`) and can be forfeited if police
catch you dirty (`gtarp_seizure`).
...
5. **Real dispatch + a `gtarp_evidence` trail** on delivery, not qbx_drugs'
...
`gtarp_smuggling_runs` (`sql/0038_smuggling.sql`) — one row per run: dropoff_id,
