...
--   - palm6_citations   (0024): fine ledger, status in unpaid/paid/escalated,
--                        columns amount, status, created_at.
--   - palm6_mdt_bookings (0023 + sealed_at from 0026): arrest paperwork,
--                        columns citizen_name, officer_name, charges,
--                        booked_at, sealed_at.
--   - palm6_mdt_calls   (0025): 911 dispatch log, columns text, src_label,
--                        created_at.
--
...
local function dbg(msg)
    if Config.Debug then print('[palm6_blotter] ' .. msg) end
end
...
local function buildSummary(hours)
    ...
    -- Recent 911 calls. palm6_mdt_calls has no status column, so "recent" is
    -- the time window, matching how palm6_mdt and palm6_ems read it.
    pcall(function()
        local r = MySQL.single.await([[
            SELECT COUNT(*) AS n
...
-- timer thread, guarded twice: the config flag and palm6_discord being
...
CreateThread(function()
    ...
    while true do
        if Config.Digest.Enabled and Bridge.ResourceStarted('palm6_discord') then
...
AddEventHandler('onResourceStart', function(resource)
    ...
    print(('[palm6_blotter] read-only civic blotter online, /blotter (police); weekly digest %s')
