-- ============================================================================
-- palm6_witnesses/client/main.lua
--
...
RegisterNetEvent('palm6_witnesses:sync', function(list, _)
...
CreateThread(function()
    Wait(5000)
    while true do
        TriggerServerEvent('palm6_witnesses:requestSync')
...
RegisterNetEvent('palm6_witnesses:beginCanvass', function(durationSec)
    if busy then return end
    busy = true
    local done = Game.ProgressBar('Talking to the witness...', durationSec * 1000)
    busy = false
    if done then
        TriggerServerEvent('palm6_witnesses:canvass:finish')
    else
        TriggerServerEvent('palm6_witnesses:canvass:cancel')
    end
end)

RegisterNetEvent('palm6_witnesses:beginPress', function(durationSec)
    if busy then return end
    busy = true
    ...
    if done then
        TriggerServerEvent('palm6_witnesses:press:finish')
    else
        TriggerServerEvent('palm6_witnesses:press:cancel')
        Game.Notify({ title = 'Witness', description = 'You backed off. They\'re still talking.', type = 'error' })
    end
end)

RegisterNetEvent('palm6_witnesses:showStatement', function(title, body)
...
-- ---------------------------------------------------------------------------
RegisterNetEvent('palm6_witnesses:captureAppearance', function(nonce)
    if type(nonce) ~= 'string' then return end
    TriggerServerEvent('palm6_witnesses:appearanceResult', nonce, Game.GetAppearanceSignature())
end)
                ...
                if nearest.role == 'police' then
                    if nearestDist <= Config.Canvass.Radius then
                        ...
                        if Game.InteractPressed() then
                            TriggerServerEvent('palm6_witnesses:canvass:start', nearest.id)
                        end
                    end
                else -- suspect
                    if nearestDist <= Config.Payoff.Radius then
                        ...
                        if Game.InteractPressed() then
                            TriggerServerEvent('palm6_witnesses:payoff', nearest.id)
                        end
                    ...
                    if nearestDist <= Config.Press.Radius and Game.AltPressed() then
                        if Game.IsArmed() and Game.IsAiming() then
                            TriggerServerEvent('palm6_witnesses:press:start', nearest.id)
                        else
