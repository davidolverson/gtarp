-- ============================================================================
-- gtarp_seizure/shared/config.lua — engine-agnostic tunables (Tier 1).
--
...
-- flows toward gtarp_laundering's wash. Nothing lets police INTERDICT it.
-- gtarp_seizure is that: an on-duty officer standing over a WANTED suspect can
-- forfeit the suspect's dirty money — it's removed from circulation, written to
-- a persistent forfeiture ledger, and attached to a gtarp_evidence case.
--
...
-- additive layer, same as gtarp_citations vs the recipe's paperless BillPlayer.
--
-- NOT gtarp_counterfeit. `counterfeit_cash` (fake money) is seized by
-- gtarp_counterfeit's own /seizefake ledger — this resource never touches it.
...
-- Probable cause: only a WANTED suspect (active gtarp_mdt warrant) can have
-- their dirty money forfeited. Ties seizure to the crime→warrant system and
-- stops officers shaking down innocent/RP-clean players. If gtarp_mdt is
