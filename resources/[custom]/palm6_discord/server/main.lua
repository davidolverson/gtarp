...
local function dbg(msg)
    if Config.Debug then print('[palm6_discord] ' .. msg) end
end
...
local function announce(feed, payload)
    ...
    if not floodOk(feed) then
        ...
        print(('[palm6_discord] feed "%s" over %d msgs/min — dropping message'):format(
        ...
        if msg then
            Bridge.HttpPostJson(msg.url, msg.body, function(status, _, headers)
                ...
                elseif status < 200 or status > 299 then
                    droppedTotal = droppedTotal + 1
                    print(('[palm6_discord] feed "%s" delivery failed (HTTP %s) — message dropped'):format(
...
AddEventHandler('onResourceStart', function(resource)
    ...
    print(('[palm6_discord] announcer online — live: %s | off: %s'):format(
