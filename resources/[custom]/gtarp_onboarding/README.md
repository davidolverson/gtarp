...
1. First character load ever (tracked per-citizenid in `gtarp_onboarding`,
...
3. First (and only first) successful accept: grants `Config.StarterCash`
   ...
   `gtarp_staff`'s audit log (queryable for ban/dispute conversations),
...
- `GetSummary()` → `{ totalAccepted: number }` — total onboarded citizens,
  all-time. Consumed by `gtarp_devtest`.
