...
RegisterNetEvent('gtarp_onboarding:acceptRules', function()
    ...
    if Config.StarterVehicle.enabled then
        ...
        if granted then
            pcall(function()
                MySQL.update.await(
                    'UPDATE gtarp_onboarding SET starter_vehicle_granted = 1 WHERE citizenid = ?',
                    { cid })
            end)
    ...
    if Config.StarterOutfit.enabled then
        if Bridge.SetStarterOutfit(src, cid) then
            pcall(function()
                MySQL.update.await(
                    'UPDATE gtarp_onboarding SET starter_outfit_granted = 1 WHERE citizenid = ?',
                    { cid })
            end)
        end
    end
