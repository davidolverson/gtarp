-- ============================================================================
-- palm6_wanted/bridge/sv_framework.lua
--
...
--
-- Copied from palm6_blotter / palm6_ems bridges and trimmed to what a
...
function Bridge.Reply(src, lines)
    if src == 0 then
        for _, line in ipairs(lines) do print('[palm6_wanted] ' .. line) end
        return
    end
    -- One palm6_ui panel instead of dumping lines into the chat feed.
    TriggerClientEvent('palm6_ui:show', src, { tag = 'Wanted', color = { 235, 120, 120 }, lines = lines })
end
