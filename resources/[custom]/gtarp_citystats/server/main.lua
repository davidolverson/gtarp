-- ============================================================================
-- gtarp_citystats/server/main.lua
--
...
--   - gtarp_gangs        (0041): COUNT(*) + SUM(vault_balance); top by rep.
--                        Confirmed columns: id, name, tag, vault_balance, rep.
--   - gtarp_gang_members (0041): COUNT(*) affiliated citizens.
--                        Confirmed column: citizenid.
--   - gtarp_drugs_sales  (0039): SUM(net_dirty) + COUNT(*) since a recent
--                        window on created_at. Confirmed columns: net_dirty,
--                        created_at.
--   - gtarp_mdt_warrants (0023): COUNT(*) WHERE status = 'active'.
...
local function dbg(msg)
    if Config.Debug then print('[gtarp_citystats] ' .. msg) end
end
...
local function buildStats(hours)
    ...
    if Config.Stats.Gangs then
        ...
        pcall(function()
            local r = MySQL.single.await(
                'SELECT COUNT(*) AS c, COALESCE(SUM(vault_balance), 0) AS v FROM gtarp_gangs')
            if r then
        ...
        pcall(function()
            stats.gangs.top = MySQL.query.await(
                'SELECT name, tag, rep FROM gtarp_gangs ORDER BY rep DESC, name ASC LIMIT ?',
    ...
    if Config.Stats.Members then
        ...
        pcall(function()
            local r = MySQL.single.await('SELECT COUNT(*) AS c FROM gtarp_gang_members')
    ...
    if Config.Stats.Warrants then
        ...
        pcall(function()
            local r = MySQL.single.await(
                "SELECT COUNT(*) AS c FROM gtarp_mdt_warrants WHERE status = 'active'")
...
AddEventHandler('onResourceStart', function(resource)
    ...
    print('[gtarp_citystats] read-only city stats online, /citystats (any citizen)')
end)
